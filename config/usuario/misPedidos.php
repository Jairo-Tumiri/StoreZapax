<?php
include 'header.php';
$sql_fetch_todos = "SELECT * FROM pedido WHERE idusuario_pedido = " . $idUsuario . " AND estado_pedido = '1'";
$query = mysqli_query($conexion, $sql_fetch_todos);

$suma_precio = "SELECT SUM(precioproducto_pedido)AS 'total' FROM pedido WHERE idusuario_pedido = '" . $idUsuario . "' AND estado_pedido = '1'";
$total_precio = mysqli_query($conexion, $suma_precio);

$tablapago = "SELECT * FROM pagos WHERE idusuario_pago = " . $idUsuario . "";
$pago = mysqli_query($conexion, $tablapago);

?>
<main class="contenedor-carrito">
    <form class="table" style="width: 40%; height: 80vh;">
        <table>
            <thead>
                <tr>
                    <th>Imagen</th>
            ...
            <tbody>
                ...
                while ($datos = mysqli_fetch_array($query)) { ?>
                    <tr>
                        ...
                        <!-- <td>
                            <a name="id_pedido" href="acciones/codBorrarCarrito.php?id_pedido=<?php //echo $datos['id_pedido'] 
                                                                                                ?>" role="button"><i class="ri-delete-bin-line delete"></i></a>
                        </td> -->
                    </tr>
                <?php $numero++;
                } ?>
    ...
    <?php if ($seguimientope = mysqli_fetch_array($query)) {
        $idd = $seguimientope['id_pedido'];
    } ?>
    <div class="total" style="width: 60%; padding: 25px">
        <h1>Tu pedido esta en camino <?php echo $nombreUsuario; ?></h1>
        <br>
        <h2>total de pago: <?php $t = mysqli_fetch_array($total_precio);
                            $total = $t['total'];
                            echo " $ " . number_format($total) ?></h2>
        <br>
        <br>
        <br>
        <a class="btn-rojo" href="acciones/codBorrarCarrito.php?borrar_pedido=<?php echo $idUsuario?>" style="padding: 6px 12px;">Cancelar Compra</a>
        <br>
        <br>
        <br><br>
        <h3>Seguimiento:</h3><br>
        <p style="font-size: 2rem;">
            <?php
            if (!isset($idd)) {
                if ($seguimiento = mysqli_fetch_array($pago)) {
                    $estadoSeguimiento = $seguimiento['estadopedido_pago'];
                }
                // $estadoSeguimiento = '-1';
